...
const Task = ({ task, fetchTasks, deleteTask, toggleReminder }) => {
  ...
  const fetchTask = async () => {
    const res = await fetch(`https://react-task-tracker-server.onrender.com/tasks/${task._id}`)
  ...
  const deleteT = async (e) => {
    e.preventDefault();
    await axios.delete(`https://react-task-tracker-server.onrender.com/tasks/${task._id}` ).then(res => {
    ...
    const reminderToggle = async (e) => {
      ...
      await axios.post(`https://react-task-tracker-server.onrender.com/update/${task._id}`, upDTask).then(res => {
        console.log(res)
